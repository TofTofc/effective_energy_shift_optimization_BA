...
def init(excess_array, deficit_array):
    ...
    #TODO: Datentypen testen damit sie ausreichen
    ...
    energy_excess = np.empty((n, initial_capacity), dtype=np.uint16)
    energy_deficit = np.empty((n, initial_capacity), dtype=np.uint16)
