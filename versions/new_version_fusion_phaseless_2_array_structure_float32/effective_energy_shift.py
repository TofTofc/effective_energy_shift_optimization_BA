...
def move_excess(current_phase_idx, next_phase_idx,
    ...
    for idx in range(start_idx, total):
        ...
        can_merge = (phase_meta[next_phase_idx, 2] > 0) and (np.isclose(excess_start, last_excess_end_height, atol=1e-6))
...
def init(excess_array, deficit_array):
    ...
    data_excess = np.empty((n, initial_capacity, 2), dtype=np.float32)
    data_deficit = np.empty((n, initial_capacity, 2), dtype=np.float32)

    phase_meta = np.zeros((n, 5), dtype=np.uint8)
    max_height_array = np.zeros(n, dtype=np.float32)
    phase_meta[:, 0] = 1
    phase_meta[:, 1] = 1
    ...
    for i in numba.prange(n):
        data_excess[i, 0, 0] = 0.0
        data_excess[i, 0, 1] = excess_array[i]
        data_deficit[i, 0, 0] = 0.0
