...
class Phase:
    ...
    def __init__(self, energy_excess: float, energy_deficit: float, id: int = None,  initial_capacity=100):
        self.id = id
    ...
    def append_excess(self, excess_start, excess_content, excess_balanced, excess_id):
        if self.size_excess >= self.capacity:
            self.capacity *= 2
            self.starts_excess = np.resize(self.starts_excess, self.capacity)
            self.energy_excess = np.resize(self.energy_excess, self.capacity)
            self.excess_balanced = np.resize(self.excess_balanced, self.capacity)
            self.excess_ids = np.resize(self.excess_ids, self.capacity)
    ...
    def append_deficit(self, new_start, energy_remaining, balanced):
        if self.size_deficit >= self.capacity:
            self.capacity *= 2
            self.starts_deficit = np.resize(self.starts_deficit, self.capacity)
            self.energy_deficit = np.resize(self.energy_deficit, self.capacity)
            self.deficit_balanced = np.resize(self.deficit_balanced, self.capacity)
    ...
    def __eq__(self, other):
        # Prüfen, ob beide Phase-Objekte die gleiche Größe haben
        ...
        # Vergleiche nur die gefüllten Teile
        return (
